...
class Neat {
	constructor(input, output, fitness, options) {
		...
		this.input = input;
		/* The output size of the networks */
		this.output = output;
	...
	selectMutationMethod(genome) {
		let mutationMethod = this.mutation[Math.floor(Math.random() * this.mutation.length)];
	...
	/* Sorts the population according to their score */
	sort() {
	...
	getParent() {
		...
		switch (this.selection) {
			...
			case selection.FITNESS_PROPORTIONATE: {
				 ...
				 * See: https://stackoverflow.com/questions/16186686/genetic-algorithm-handling-negative-fitness-values
