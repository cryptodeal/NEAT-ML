...
class Node {
	...
	activate(input) {
		...
		for (i = 0; i < this.connections.gated.length; i++) {
			...
			if (idx > -1) {
				influences[idx] += conn.weight * conn.from.activation;
			} else {
				nodes.push(node);
				influences.push(
		...
		for (i = 0; i < this.connections.in.length; i++) {
			...
			for (let j = 0; j < nodes.length; j++) {
				...
				let idx = connection.xtrace.nodes.indexOf(node);

				if (idx > -1) {
					connection.xtrace.values[idx] =
	...
	ungate(connections) {
		...
		for (let i = connections.length - 1; i >= 0; i--) {
			...
			let idx = this.connections.gated.indexOf(connection);
			this.connections.gated.splice(idx, 1);
